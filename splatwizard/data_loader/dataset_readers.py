...
from splatwizard.utils.graphics_utils import getWorld2View2, focal2fov, fov2focal, BasicPointCloud
import numpy as np
import json
from pathlib import Path
from plyfile import PlyData, PlyElement
from splatwizard.utils.sh_utils import SH2RGB
...
class CameraInfo(NamedTuple):
    ...
    qvec: np.array = None
    tvec: np.array = None
    scale_mat: np.array = None
    world_mat: np.array = None
...
def read_colmap_cameras(cam_extrinsics, cam_intrinsics, images_folder, scale_mats = None, world_mats = None):
    cam_infos = []
    for idx, key in enumerate(cam_extrinsics):  # key is image_id
        # sys.stdout.write('\r')
        # # the exact output you're looking for:
        # sys.stdout.write("Reading camera {}/{}".format(idx+1, len(cam_extrinsics)))
        # sys.stdout.flush()
        # logger.info("Reading camera {}/{}".format(idx+1, len(cam_extrinsics)))
        ...
        scale_mat = scale_mats[key] if scale_mats is not None else None
        world_mat = world_mats[key] if world_mats is not None else None
        ...
        cam_info = CameraInfo(
            uid=uid, ex_id=extr.id, R=R, T=T, FovY=FovY, FovX=FovX, image=None,
...
def readColmapSceneInfo(path, images, data_mode: DataMode, llffhold=8, lod=None):
    try:
    ...
    try:
        cam_file = os.path.join(path, 'cameras.npz')
        camera_dict = np.load(cam_file)
        scale_mats = {image_id: camera_dict['scale_mat_%d' % (image_id - 1)].astype(np.float32) for image_id, v in cam_extrinsics.items()}
        world_mats = {image_id: camera_dict['world_mat_%d' % (image_id - 1)].astype(np.float32) for image_id, v in cam_extrinsics.items()}
        logger.info("Found cameras.npz, scale_mat and worldmat loaded")
    ...
    cam_infos_unsorted = read_colmap_cameras(
        ...
        scale_mats=scale_mats, world_mats=world_mats,
    )
    ...
    elif data_mode == DataMode.SPLIT_LOD:
...
def readNerfSyntheticInfo(path, white_background, data_mode, extension=".png", ply_path=None):
